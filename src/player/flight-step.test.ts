...
function simulate(opts: {
  ...
  for (let t = 0; t < seconds; t += dt) {
    const next = flightStep(position, velocity, { forward, thrust, flare, bank, hover: false, tuck: false }, dt, C)
...
describe('flightStep', () => {
  it('does not mutate the position or velocity it is given', () => {
    ...
    flightStep(position, velocity, {
      forward: new Vector3(0, 0, -1), thrust: false, flare: false, bank: 0, hover: false, tuck: false,
    }, 1 / 60, C)
  ...
  it('does not mutate the position or velocity it is given, with a non-zero bank', () => {
    ...
    const velocity = new Vector3(0, 0, -20)
    flightStep(position, velocity, {
      forward: new Vector3(0, 0, -1), thrust: false, flare: false, bank: 0.7, hover: false, tuck: false,
    }, 1 / 60, C)
    expect(position.toArray()).toEqual([0, 100, 0])
    expect(velocity.toArray()).toEqual([0, 0, -20])
  })
...
function simulateFrom(opts: {
  ...
  const dt = 1 / 60
  for (let t = 0; t < seconds; t += dt) {
    const next = flightStep(position, velocity, { forward, thrust, flare: false, bank, hover: false, tuck: false }, dt, C)
    position = next.position
    velocity = next.velocity
  }
