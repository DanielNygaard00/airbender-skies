 ...
 * asserted and false.
 *
 * The margin is small and worth stating in motes rather than in decimals. The worst case is the
 * amphitheatre lid at 0.649, which sits partly behind the hoodoo caps from the floor beneath it and
 * is meant to, so there is no headroom left to win without moving the level. `moteCount` gives that
 * lid 265 motes, so 0.049 of margin is about **thirteen motes** of occlusion. It was three when the
 * count was a flat 100 for every feature, which is one of the quieter benefits of scaling the count
 * with the feature: a bigger feature is now measured with a finer instrument as well as drawn with
 * a denser one.
 *
 * Nothing here is noisy — the island geometry and the mote scatter are both seeded — so a small
 * margin costs no flakiness, only sensitivity, which is the intended behaviour of holding a rule
